...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'process_payout') {
        ...
        if ($payoutId > 0 && !empty($transactionId)) {
            $stmt = $conn->prepare("UPDATE seller_payouts SET status = 'paid', transaction_id = ?, process_date = NOW() WHERE payout_id = ? AND status = 'pending'");
            ...
            if ($stmt->execute()) {
                ...
                $fetchStmt = $conn->prepare("
                    SELECT u.email, u.firstname, p.amount 
                ...
                if ($userRow = $fetchRes->fetch_assoc()) {
                    sendPayoutNotificationEmail($userRow['email'], $userRow['firstname'], $userRow['amount'], $transactionId, false);
                    
                    // Insert into notifications table
                    $notifStmt = $conn->prepare("INSERT INTO notifications (user_id, sender_id, type, content) VALUES ((SELECT user_id FROM sellers WHERE id = (SELECT seller_id FROM seller_payouts WHERE payout_id = ?)), ?, 'payment_confirmed', ?)");
                    $notifContent = "Your payout of ₱" . number_format($userRow['amount'], 2) . " has been sent to your E-Wallet.";
        ...
        $payoutId = intval($_POST['payout_id'] ?? 0);
        if ($payoutId > 0) {
            $stmt = $conn->prepare("UPDATE seller_payouts SET status = 'rejected', process_date = NOW() WHERE payout_id = ? AND status = 'pending'");
...
$pendingPayouts = [];
$res = $conn->query("
    SELECT p.*, s.shop_name, u.firstname, u.lastname, u.email 
    FROM seller_payouts p
    JOIN sellers s ON p.seller_id = s.id
    JOIN users u ON s.user_id = u.id
...
<head>
    ...
    <title>Seller Payouts - Admin Dashboard</title>
    <!-- FontAwesome -->
...
<body>
    ...
    <main class="main-content">
        <h1 class="page-title">Seller Payout Requests</h1>
        <p class="page-subtitle">Process seller earnings withdrawals.</p>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <span style="font-size: 12px; color: #6b7280;"><?php echo htmlspecialchars($p['firstname'] . ' ' . $p['lastname']); ?></span>
                                    </td>
                                    <td style="font-weight: 700; color: #047857;">₱<?php echo number_format($p['amount'], 2); ?></td>
                                    <td>
                                        <span style="font-weight: 600;"><?php echo htmlspecialchars($p['payout_method']); ?></span><br>
        ...
        <div class="modal-box">
            ...
            <div class="payout-details-box">
                <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
                    <span style="font-size: 13px; color: #6b7280;">Shop:</span>
                    <strong id="modalShopName">Shop Name</strong>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
    ...
    <script>
        function openProcessModal(data) {
            document.getElementById('modalPayoutId').value = data.payout_id;
            ...
            document.getElementById('modalMethod').innerText = data.payout_method;
